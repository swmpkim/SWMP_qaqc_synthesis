...
dat <- import_local(path = here::here("data"), 
                    ...
                    trace = TRUE)
# forgot to filter out diel samples!!! probably only want grabs....
...
ggplot(filter(qaqc_monthly, parameter == "f_po4f")) +
    geom_col(aes(x = Year, y = n,
                 fill = flag_detailed)) +
    facet_wrap(~Month) +
    theme_bw() +
    scale_fill_manual(values = cols) +
    labs(title = "Bangs Lake PO4 QAQC flags, faceted by month")

ggplot(filter(qaqc_monthly, parameter == "f_no23f")) +
    geom_col(aes(x = Year, y = n,
                 fill = flag_detailed)) +
    facet_wrap(~Month) +
    theme_bw() +
    scale_fill_manual(values = cols) +
    labs(title = "Bangs Lake NO23 QAQC flags, faceted by month")

ggplot(filter(qaqc_monthly, parameter == "f_chla_n")) +
    geom_col(aes(x = Year, y = n,
                 fill = flag_detailed)) +
    facet_wrap(~Month) +
    theme_bw() +
    scale_fill_manual(values = cols) +
    labs(title = "Bangs Lake Chl a QAQC flags, faceted by month")
...
# need to play with width for nutrient samples

p <- ggplot(filter(test2, parameter == "f_po4f")) +
    geom_tile(aes(x = frac.year, y = Year, fill = flag_detailed),
              width = 0.03) +
    scale_fill_manual(values = cols) +
    labs(title = "Bangs Lake PO4 flags",
...
p2 <- ggplot(filter(test2, parameter == "f_no23f")) +
    geom_tile(aes(x = frac.year, y = Year, fill = flag_detailed),
              width = 0.03) +
    scale_fill_manual(values = cols) +
    labs(title = "Bangs Lake NO23 flags",
         x = "Part of Year (0 = Jan 1; 1 = Dec 31)",
         y = "Year",
         fill = "QA/QC Flag") +
    theme_bw()
...
p3 <- ggplot(filter(test2, parameter == "f_chla_n")) +
    geom_tile(aes(x = frac.year, y = Year, fill = flag_detailed),
              width = 0.03) +
    scale_fill_manual(values = cols) +
    labs(title = "Bangs Lake Chl a flags",
         x = "Part of Year (0 = Jan 1; 1 = Dec 31)",
         y = "Year",
         fill = "QA/QC Flag") +
    theme_bw()
